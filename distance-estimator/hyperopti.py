...
def create_model(x_train, y_train, x_test, y_test):
    ...
    earlyStopping = EarlyStopping(
        monitor='loss', patience=10, verbose=0, mode='min')
    reduce_lr_loss = ReduceLROnPlateau(
        monitor='loss', factor=0.5, patience=7, verbose=1, epsilon=1e-5, mode='min')
    tensorboard = TensorBoard(log_dir="logs/model@{}".format(int(time.time())))

    # start training
    model.fit(x_train, y_train, batch_size={{choice([1024, 2048])}}, epochs=5000, callbacks=[
              tensorboard], verbose=1, validation_split=0.1)
...
if __name__ == '__main__':
    ...
    best_run, best_model = optim.minimize(
        model=create_model, data=data, algo=tpe.suggest, max_evals=3, trials=trials, eval_space=True)
