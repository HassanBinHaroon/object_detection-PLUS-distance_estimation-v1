...
----------------------------------------------------------------------------
   1    type         Describes the type of object (e.g. 'Car', 'Truck')
...
df = pd.DataFrame(columns=['filename', 'class', 'truncated', 'occluded', 'observation angle',
                  'xmin', 'ymin', 'xmax', 'ymax', 'height', 'width', 'length', 'xloc', 'yloc', 'zloc', 'rot_y'])
...
if __name__ == "__main__":
    ...
    argparser.add_argument('-fn', '--filename', help='output csv file name')
    ...
    for idx, f in enumerate(all_files):
        pbar.update(1)
        file_object = open(input_dir + f, 'r')
